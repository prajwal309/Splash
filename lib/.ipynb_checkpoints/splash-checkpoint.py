...
class Target:
    ...
    def PreClean(self, CutOff=7, NIter=2, Columns=-1, MinDataCount=50,
        ...
        for NightNumber in range(self.NumberOfNights):
            ...
            f_std = np.std(CurrentFlux)
            f_med = np.median(CurrentFlux)
            print('f_std', f_std)
            outlier_f = []
            outlier_t = []
            for t, f in zip(CurrentTime, CurrentFlux):
                if f > f_med + 2.5 * f_std:
                    outlier_f.append(f)
                    outlier_t.append(t)
            ...
            if SavePlot or ShowPlot:
                ...
                plt.plot(outlier_t-T0_Int, outlier_f, "cx", markersize=18, label="Outlier Data")
